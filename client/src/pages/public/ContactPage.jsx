import { motion } from 'framer-motion';

import InquiryForm from '../../components/forms/InquiryForm.jsx';
import MapEmbed from '../../components/property/MapEmbed.jsx';
import { sectionVariants, heroVariants, containerVariants } from '../../styles/animations.js';

// Feature: hanuvansh-mern-estate
// ContactPage: public contact page with inquiry form, WhatsApp CTA, Google Maps,
// social media links, and contact information card.
// Requirements: 14.1, 14.2, 14.3, 14.4, 14.5

// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
const OFFICE_LAT = 23.0225;
const OFFICE_LNG = 72.5714;

const CONTACT_INFO = {
  phone: '+91 98765 43210',
  email: 'info@hanuvansh.com',
  address: 'Ahmedabad, Gujarat, India',
};

const WHATSAPP_URL = 'https://wa.me/919106788526';

const SOCIAL_LINKS = [
  {
    id: 'instagram',
    label: 'Instagram',
    href: '#',
    icon: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
        fill="currentColor"
        className="w-5 h-5"
        aria-hidden="true"
      >
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
      </svg>
    ),
    color: 'hover:text-pink-400 hover:border-pink-400/40',
  },
  {
    id: 'facebook',
    label: 'Facebook',
    href: '#',
    icon: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
        fill="currentColor"
        className="w-5 h-5"
        aria-hidden="true"
      >
        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
      </svg>
    ),
    color: 'hover:text-blue-400 hover:border-blue-400/40',
  },
  {
    id: 'linkedin',
    label: 'LinkedIn',
    href: '#',
    icon: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
        fill="currentColor"
        className="w-5 h-5"
        aria-hidden="true"
      >
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
      </svg>
    ),
    color: 'hover:text-blue-300 hover:border-blue-300/40',
  },
  {
    id: 'youtube',
    label: 'YouTube',
    href: '#',
    icon: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
        fill="currentColor"
        className="w-5 h-5"
        aria-hidden="true"
      >
        <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
      </svg>
    ),
    color: 'hover:text-red-400 hover:border-red-400/40',
  },
];

// ---------------------------------------------------------------------------
// SocialButton
// ---------------------------------------------------------------------------
function SocialButton({ label, href, icon, color }) {
  return (
    <a
      href={href}
      target="_blank"
      rel="noopener noreferrer"
      aria-label={`Visit our ${label} page`}
      className={`flex items-center gap-2 px-4 py-3 rounded-lg border border-white/10 text-text-muted transition-all duration-200 ${color}`}
    >
      {icon}
      <span className="text-sm font-medium">{label}</span>
    </a>
  );
}

// ---------------------------------------------------------------------------
// ContactInfoItem
// ---------------------------------------------------------------------------
function ContactInfoItem({ icon, label, value, href }) {
  const content = (
    <div className="flex items-start gap-4">
      <div
        className="w-10 h-10 rounded-lg flex items-center justify-center text-lg flex-shrink-0 mt-0.5"
        style={{ background: 'rgba(212,175,55,0.1)', border: '1px solid rgba(212,175,55,0.3)' }}
        aria-hidden="true"
      >
        {icon}
      </div>
      <div>
        <p className="text-text-muted text-xs uppercase tracking-widest mb-1">{label}</p>
        <p className="text-text-primary text-sm font-medium">{value}</p>
      </div>
    </div>
  );

  if (href) {
    return (
      <a
        href={href}
        className="block hover:opacity-80 transition-opacity duration-200"
        aria-label={`${label}: ${value}`}
      >
        {content}
      </a>
    );
  }

  return <div>{content}</div>;
}

// ---------------------------------------------------------------------------
// ContactPage
// ---------------------------------------------------------------------------
export default function ContactPage() {
  return (
    <main className="bg-bg-primary text-text-primary font-body">
      {/* ================================================================
          1. HERO BANNER
      ================================================================ */}
      <section
        className="relative min-h-[50vh] flex items-center justify-center overflow-hidden"
        style={{ background: 'linear-gradient(135deg, #0a0a0a 0%, #111111 60%, #0a0a0a 100%)' }}
        aria-label="Contact Us Hero"
      >
        {/* Decorative radial glow */}
        <div
          className="absolute inset-0 pointer-events-none"
          style={{
            background:
              'radial-gradient(ellipse 70% 60% at 50% 50%, rgba(212,175,55,0.07) 0%, transparent 70%)',
          }}
          aria-hidden="true"
        />

        {/* Decorative corner accents */}
        <div
          className="absolute top-0 left-0 w-32 h-32 pointer-events-none opacity-30"
          style={{ background: 'linear-gradient(135deg, rgba(212,175,55,0.4) 0%, transparent 60%)' }}
          aria-hidden="true"
        />
        <div
          className="absolute bottom-0 right-0 w-32 h-32 pointer-events-none opacity-30"
          style={{ background: 'linear-gradient(315deg, rgba(212,175,55,0.4) 0%, transparent 60%)' }}
          aria-hidden="true"
        />

        <motion.div
          variants={heroVariants}
          initial="initial"
          animate="animate"
          className="relative z-10 text-center px-6 max-w-3xl mx-auto"
        >
          {/* Gold accent line */}
          <div className="flex items-center justify-center gap-4 mb-6">
            <div className="h-px w-16 bg-gold opacity-60" />
            <span className="text-gold text-xs tracking-[0.3em] uppercase font-semibold">
              Get In Touch
            </span>
            <div className="h-px w-16 bg-gold opacity-60" />
          </div>

          <h1 className="font-heading text-4xl sm:text-5xl md:text-6xl font-bold text-text-primary leading-tight mb-6">
            Contact <span className="text-gold">Us</span>
          </h1>

          <p className="text-text-muted text-sm sm:text-base tracking-[0.15em] uppercase max-w-xl mx-auto">
            We're here to guide you every step of the way
          </p>

          {/* Bottom gold accent */}
          <div className="flex items-center justify-center gap-4 mt-8">
            <div className="h-px w-8 bg-gold opacity-40" />
            <div className="w-1.5 h-1.5 rounded-full bg-gold opacity-60" />
            <div className="h-px w-8 bg-gold opacity-40" />
          </div>
        </motion.div>
      </section>

      {/* ================================================================
          2. INQUIRY FORM + CONTACT INFO
      ================================================================ */}
      <section
        className="py-20 px-6 bg-bg-secondary"
        aria-label="Contact Form and Information"
      >
        <div className="max-w-6xl mx-auto">
          <motion.div
            variants={sectionVariants}
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true }}
            className="text-center mb-14"
          >
            <p className="text-accent text-xs tracking-[0.3em] uppercase font-semibold mb-3">
              Reach Out
            </p>
            <h2 className="font-heading text-3xl sm:text-4xl font-bold text-text-primary mb-4">
              Send Us an <span className="text-gold">Inquiry</span>
            </h2>
            <div className="h-px w-20 bg-gold mx-auto opacity-60" />
          </motion.div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            {/* Inquiry Form — no propertyId for general inquiries */}
            <motion.div
              variants={sectionVariants}
              initial="hidden"
              whileInView="visible"
              viewport={{ once: true }}
            >
              <InquiryForm />
            </motion.div>

            {/* Contact Info Card + WhatsApp + Social */}
            <motion.div
              variants={sectionVariants}
              initial="hidden"
              whileInView="visible"
              viewport={{ once: true }}
              className="space-y-6"
            >
              {/* Contact Information Card */}
              <div
                className="bg-bg-card border border-white/10 rounded-xl p-8 space-y-6"
                aria-label="Contact Information"
              >
                <div className="flex items-center gap-3 mb-2">
                  <div className="w-1 h-8 bg-gold rounded-full" aria-hidden="true" />
                  <h3 className="font-heading text-xl font-bold text-gold">Our Details</h3>
                </div>

                <ContactInfoItem
                  icon="📞"
                  label="Phone"
                  value={CONTACT_INFO.phone}
                  href={`tel:${CONTACT_INFO.phone.replace(/\s/g, '')}`}
                />

                <ContactInfoItem
                  icon="✉️"
                  label="Email"
                  value={CONTACT_INFO.email}
                  href={`mailto:${CONTACT_INFO.email}`}
                />

                <ContactInfoItem
                  icon="📍"
                  label="Office Address"
                  value={CONTACT_INFO.address}
                />
              </div>

              {/* WhatsApp CTA */}
              <a
                href={WHATSAPP_URL}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Chat with us on WhatsApp"
                className="flex items-center justify-center gap-3 w-full py-4 rounded-xl font-semibold text-white transition-all duration-200 hover:opacity-90 active:scale-95"
                style={{ background: 'linear-gradient(135deg, #25D366 0%, #128C7E 100%)' }}
              >
                {/* WhatsApp icon */}
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                  className="w-6 h-6"
                  aria-hidden="true"
                >
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
                <span>Chat on WhatsApp</span>
              </a>

              {/* Social Media Links */}
              <div
                className="bg-bg-card border border-white/10 rounded-xl p-6"
                aria-label="Social Media Links"
              >
                <div className="flex items-center gap-3 mb-5">
                  <div className="w-1 h-6 bg-accent rounded-full" aria-hidden="true" />
                  <h3 className="font-heading text-base font-bold text-accent">Follow Us</h3>
                </div>
                <motion.div
                  variants={containerVariants}
                  initial="hidden"
                  whileInView="visible"
                  viewport={{ once: true }}
                  className="grid grid-cols-2 gap-3"
                >
                  {SOCIAL_LINKS.map((social) => (
                    <SocialButton
                      key={social.id}
                      label={social.label}
                      href={social.href}
                      icon={social.icon}
                      color={social.color}
                    />
                  ))}
                </motion.div>
              </div>
            </motion.div>
          </div>
        </div>
      </section>

      {/* ================================================================
          3. GOOGLE MAPS SECTION
      ================================================================ */}
      <section
        className="py-20 px-6 bg-bg-primary"
        aria-label="Office Location Map"
      >
        <div className="max-w-5xl mx-auto">
          <motion.div
            variants={sectionVariants}
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true }}
            className="text-center mb-12"
          >
            <p className="text-gold text-xs tracking-[0.3em] uppercase font-semibold mb-3">
              Find Us
            </p>
            <h2 className="font-heading text-3xl sm:text-4xl font-bold text-text-primary mb-4">
              Our <span className="text-gold">Location</span>
            </h2>
            <div className="h-px w-20 bg-gold mx-auto opacity-60 mb-4" />
            <p className="text-text-muted text-sm">{CONTACT_INFO.address}</p>
          </motion.div>

          <motion.div
            variants={sectionVariants}
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true }}
          >
            <MapEmbed
              lat={OFFICE_LAT}
              lng={OFFICE_LNG}
              title="Hanuvansh Estate Consultant — Ahmedabad Office"
            />
          </motion.div>
        </div>
      </section>
    </main>
  );
}
